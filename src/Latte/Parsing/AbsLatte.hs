...
module Latte.Parsing.AbsLatte where
...
data TopDef a
    ...
    | StructDef a Ident [StructBody a]
  deriving (Eq, Ord, Show, Read)

instance Functor TopDef where
    fmap f x = case x of
        FnDef a type_ ident args block -> FnDef (f a) (fmap f type_) ident (map (fmap f) args) (fmap f block)
        StructDef a ident structbodys -> StructDef (f a) ident (map (fmap f) structbodys)
data StructBody a = Attr a (Type a) Ident
  deriving (Eq, Ord, Show, Read)

instance Functor StructBody where
    fmap f x = case x of
...
data Expr a
    ...
    | ELitFalse a
    | EString a String
    | EStructCoerce a (Expr a)
    | EStructArrCoerce a (ArrayType a)
    | EApp a Ident [Expr a]
    | EStructField a (Expr a) Ident
...
instance Functor Expr where
    fmap f x = case x of
        ...
        ENewStruct a type_ -> ENewStruct (f a) (fmap f type_)
        EVar a ident -> EVar (f a) ident
        ELitInt a integer -> ELitInt (f a) integer
        ELitTrue a -> ELitTrue (f a)
        ELitFalse a -> ELitFalse (f a)
        EString a string -> EString (f a) string
        EStructCoerce a expr -> EStructCoerce (f a) (fmap f expr)
        EStructArrCoerce a arraytype -> EStructArrCoerce (f a) (fmap f arraytype)
        EApp a ident exprs -> EApp (f a) ident (map (fmap f) exprs)
        EStructField a expr ident -> EStructField (f a) (fmap f expr) ident
